...
func SendVerificationEmailWithOtp(otp int, recieverEmail string, recieverName string) error {
	...
	subject := "Verify Your Email Address for Explorite"
	body := fmt.Sprintf("Hello,%s\n\nThank you for signing up for Explorite. To complete your registration and ensure the security of your account, please verify your email address by entering the One-Time Password (OTP) provided below:\n\nOTP: %d\n\nPlease use the OTP to verify your email address on our platform within the next 10 minutes. After this time, the OTP will expire, and you will need to request a new one.\n\nIf you did not request this verification, please disregard this email.\n\nIf you need any assistance or have questions, feel free to reach out to our support team at support@example.com.\n\nThank you for choosing Explorite.\n\nBest regards,\nThe Explorite Team", recieverName, otp)
	...
	fmt.Println("pritntttt", smtpHost)
	fmt.Println("pritntttt", smtpPort)
	fmt.Println("pritntttt", auth)
	fmt.Println("pritntttt", from)
	fmt.Println("pritntttt", to)
...
func SendRestPasswordEmailOtp(otp int, recieverEmail string) error {
	...
	body := fmt.Sprintf("Dear %s,\n\nYou recently requested to reset your password for your Explorite account. To complete the process, please use the following One-Time Password (OTP):\n\nOTP: %d\n\nThis OTP is valid for 10 minutes. Please do not share this OTP with anyone for security reasons. If you did not request a password reset, please ignore this email.\n\nThank you,\nThe Explorite Team", recieverEmail, otp)
